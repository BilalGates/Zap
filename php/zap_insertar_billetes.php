...
<html lang="es">
    <head>
        ...
        <title>ZAP · Insertar billetes</title>
    </head>
    <body>
        ...
        <div class="form">
            <h2>Insertar billetes</h2>
            <form method="POST" action="../php/zap_insertar_billetes.php">
                <label>Tipus:
            ...
            <?php 
                if (isset($_POST["Insertar"])) {
                    if (empty($_POST["tipus"]) or empty($_POST["dia"]) or empty($_POST["estat"]) or empty($_POST["viatgers"]) or empty($_POST["viatge"])) {
                        ...
                        if ($fila[0] == 1) {
                            $sentencia = "INSERT INTO bitllets (Tipus, Dia, Estat, Viatgers, ID_viatge) VALUES ( '" . $_POST["tipus"] . "', '" . $_POST["dia"] . "', '" . $_POST["estat"] . "', '" . $_POST["viatgers"] . "', '" . $_POST["viatge"] . "')" ;
                            $result=mysqli_query($enlace, $sentencia);
                            if (!$result) {
                                echo "<div class='message error'>El registro no se ha podido insertar.</div>";
                            } else {
                                echo "<div class='message success'>Billete insertado correctamente.</div>";
                            }
                        } else {
