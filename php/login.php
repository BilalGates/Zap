<?php
    if (isset($_POST["boton"])) {
        ...
        if (empty($usuario) or empty($contrasena)) {
            echo "<script>alert('Introduce los valores'); window.location.href = '../html/login.html';</script>";
        } else {
            $servidor = "localhost";
            $user = "root";
            $password = "";
            ...
            if ($fila[0] == 1) {
                ...
                echo "<script>alert('Error al autenticarse'); window.location.href = '../html/login.html';</script>";
            }
