...
<html lang="es">
    ...
    <body>
        ...
        <div class="form">
            ...
            <form method="POST" action="../php/zap_modificar_viajes.php">
                <label>ID del viaje:
                <input type="text" name="Tid" size=50></label> <br>
                <label>Tren:
                <input type="text" name="Ttren"></label> <br>   
                <label>Origen:
                <input type="text" name="Torigen" size=50></label> <br>
                <label>Destino:
                <input type="text" name="Tdestino" size=50></label> <br>
                <label>Día:
                <input type="date" name="Tdia" size=50></label> <br>
                <label>Hora:
                <input type="time" name="Thora" size=50></label> <br>
            ...
            <?php 
                if (isset($_POST["Modificar"])) {
                    if (empty($_POST["Tid"]) or empty($_POST["Ttren"]) or empty($_POST["Torigen"]) or empty($_POST["Tdestino"]) or empty($_POST["Tdia"]) or empty($_POST["Thora"]) or empty($_POST["Tidestacion"]) or empty($_POST["Tidcochera"])) {
                        ...
                        $sentenciaS = "SELECT count(*) FROM viatges WHERE ID_viatge = '" . $_POST["Tid"] . "'";
                        ...
                        if ($filaE[0] == 0 && $filaC[0] == 0) {
                            echo "<br> Los campos de 'Id de la estación' e 'Id de la cochera' son incorrectos.";
                        } else {
                            if ($filaE[0] == 0) {
                                ...
                                if ($filaC[0] == 0) {
                                    ...
                                    if ($fila[0] == 0) {
                                        ...
                                        $sentencia = "UPDATE viatges SET Tren = '" . $_POST["Ttren"] . "', Origen = '" . $_POST["Torigen"] . "', Desti = '" . $_POST["Tdestino"] . "', Dia_hora = '" . $_POST["Tdia"] . $_POST["Thora"] . "', ID_estacio = '" . $_POST["Tidestacion"] . "', ID_cotxeres = '" . $_POST["Tidcochera"]  . "'" . " WHERE ID_viatge = '" . $_POST["Tid"] . "'";
                                    ...
                                    if (!$result) {
                                        ...
                                        echo "<div class='message success'>Los datos del viaje se han modificado correctamente.</div>";
                                    }
                                }
                            }
                        }
